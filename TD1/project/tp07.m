...
##colorbar;
##colormap jet;
##title(['Potentiel après ', num2str(k), ' itérations (tolérance = ', num2str(tol), ' V)']);
##xlabel('x (i)');
##ylabel('y (j)');
##axis equal tight;
##
...
##figure;
##contour(V', 20, 'LineWidth', 1.5);
##colorbar;
##colormap jet;
##title('Lignes équipotentielles');
##xlabel('x (i)');
##ylabel('y (j)');
##axis equal tight;
...
% (On transpose les champs et coordonnées, sans changer leur signe)
Ex_plot = Ex';    % transpose uniquement
Ey_plot = Ey';    % transpose uniquement
x_plot  = x';
y_plot  = y';


##% Superposition du champ E sur le potentiel
##figure;
##pcolor(V');
##shading interp;
##hold on;
...
##colorbar;
##colormap jet;
##title('Potentiel et champ électrique \bfE');
##xlabel('x (i)');
##ylabel('y (j)');
##axis equal tight;

% Calcul capacité avec un contour autour d'un conducteurs

#contour pour un conducteur
contour1_i = 6:34;
contour1_j = 24:29;

#Somme des flux
Q = -sum(epsilon0 * Ex(contour1_i,24)) + sum(epsilon0 * Ex(contour1_i,29))
    -sum(epsilon0 * Ey(contour1_j,6)) + sum(epsilon0 * Ey(contour1_j,34));

% Capacité
Vdiff = v1 - v2;
C = Q / Vdiff;

fprintf('Capacité estimée du condensateur : %.3e F\n', C);
