...
for n = 1:max_time
    ...
    if mod(n,20)==0
        ...
        axis([0 L -1.1 1.1])
        yticks(-1.1:0.1:1.1)      % graduations verticales fines

        title(['Propagation avec slab diélectrique   t = ' num2str(t*1e9,'%.1f') ' ns'])
        xlabel('z (m)'), ylabel('E_x (V/m)')
        hold on
        % Indiquer le slab diélectrique en fond gris
        fill([dielec_deb*dz dielec_fin*dz dielec_fin*dz dielec_deb*dz], [-1.1 -1.1 1.1 1.1], [0.8 0.8 0.8], 'FaceAlpha', 0.3, 'EdgeColor', 'none')
        hold off
        pause(0.01)
    end
